...
const AddItem = () => {
  ...
  return <div className="mt-5 container text-center">
    ...
    <Modal centered show={show} onHide={() => setShow(false)}>
      ...
      <Modal.Body>Are you sure you want to Save product!</Modal.Body>
      <Modal.Footer>
        <Button variant="secondary" onClick={() => { if (!submit) { setShow(false) } }}>
          Close
        </Button>
        <Button variant="success" onClick={async () => { await add(); if (!error) { setShow(false) } }}>
          Save Changes  {submit && <Spinner size="sm" className="ms-2" animation="border" role="status">
